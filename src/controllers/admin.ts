...
import {
  ...
  statusEnum,
} from "../entities/Business_register_business";
...
export const getAllPendingBusinessRegistrations = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const getOnePendingBusinessRegistration = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const pendingBusinessRegistration =
      await Business_register_business.findOneBy({ registration_id: id }); // Fetch one pending business
...
export const reviewOnePendingBusinessRegistration = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const getAllAvatarRequests = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    const avatarRequests = await Business_avatar_request.find(); // Fetch all customers
    res.status(200).json(avatarRequests);
  } catch (error) {
    console.log(error);
    res.status(500).json({
      status: 500,
      message: "Failed to fetch avatar requests",
    });
  }
};

export const getOneAvatarRequest = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    const { registration_id } = req.params;
    const id = parseInt(registration_id);
    const avatarRequest = await Business_avatar_request.findOneBy({
      registration_id: id,
    }); // Fetch one customer
    res.status(200).json(avatarRequest);
  } catch (error) {
    console.log(error);
    res.status(500).json({
      status: 500,
      message: "Failed to fetch avatar request",
    });
  }
};
